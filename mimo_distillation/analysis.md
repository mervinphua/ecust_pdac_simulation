...
$$\begin{bmatrix} T_{17}(s) \\ T_4(s) \end{bmatrix} = \begin{bmatrix} \frac{-2.16 e^{-1.0s}}{8.25s+1} & \frac{1.26 e^{-0.3s}}{7.05s+1} \\ \frac{-2.75 e^{-1.8s}}{8.25s+1} & \frac{4.28 e^{-0.35s}}{9.0s+1} \end{bmatrix} \begin{bmatrix} R(s) \\ S(s) \end{bmatrix}$$
...
| 元素 | 传递函数 | 含义 |
|------|----------|------|
| $G_{11}$ | $\frac{-2.16\,e^{-1.0s}}{8.25s+1}$ | $R \to T_{17}$（对角，Loop 1） |
| $G_{12}$ | $\frac{1.26\,e^{-0.3s}}{7.05s+1}$ | $S \to T_{17}$（耦合项） |
| $G_{21}$ | $\frac{-2.75\,e^{-1.8s}}{8.25s+1}$ | $R \to T_4$（耦合项） |
| $G_{22}$ | $\frac{4.28\,e^{-0.35s}}{9.0s+1}$ | $S \to T_4$（对角，Loop 2） |
...
对于一阶惯性+纯延迟（FOPDT）模型 $G(s) = \frac{K e^{-\theta s}}{\tau s + 1}$，闭环临界稳定条件为：
