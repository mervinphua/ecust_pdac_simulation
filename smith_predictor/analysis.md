...
- 被控过程：$G_p(s) = \dfrac{e^{-\theta s}}{5s+1}$，其中 $\theta = 1$
- 执行器：$G_v = 1$
- 测量变送器：$G_m = 1$
...
**示例**：对于 $G_p(s) = \dfrac{e^{-s}}{5s+1}$，即使使用纯比例控制 $G_c = K_c$，特征方程为：
...
| 环节 | 传递函数 | 说明 |
|------|----------|------|
| 被控过程 $G_p(s)$ | $\dfrac{e^{-\theta s}}{5s+1}$, $\theta=1$ | 一阶惯性 + 纯延迟 |
| 执行器 $G_v$ | $1$ | 理想执行器 |
| 测量变送器 $G_m$ | $1$ | 理想测量 |
...
- **无延迟模型**（Delay-free model）：$G_p^*(s) = \dfrac{1}{5s+1}$
- **纯延迟**：$e^{-\theta s},\ \theta = 1$
